# Business question: Vostok rocket — mission outcome extraction and pie visualization
...
**Scope — Vostok rocket:** Restrict the analysis to missions where **`Rocket` equals exactly `Vostok`** (string match on the `Rocket` column). Do **not** merge in rows whose `Rocket` is a different label (for example `Vostok-2` is a separate value in this dataset and is **out of scope** unless the retrieved context explicitly includes a documented broader filter).
...
1. **Select fields** — Use at least `Rocket`, `MissionStatus`, and `Mission` (or `Date`) so each counted row is identifiable; justify any **additional** filters beyond `Rocket` = `Vostok` (for example date range or `Location`) only if they appear in the question or retrieved sample.
...
3. **Extract** — From **retrieved context only**, count rows per `MissionStatus` among rows with **`Rocket` = `Vostok`** (and total rows with non-empty `MissionStatus` in that slice). Do not infer totals for the full CSV unless every row needed is present in context.
4. **Self-critique** — For each slice, state whether the count is **directly supported** by cited rows or columns; flag if retrieval might omit part of the Vostok population.
...
```mermaid
pie showData
    title Outcome mix — Vostok rocket (replace labels and values from your extraction)
    "Success" : 0
    "Failure" : 0
    "Partial Failure" : 0
    "Prelaunch Failure" : 0
...
(Replace zeros with values **computed from provided records only**, after applying the **`Rocket` = `Vostok`** filter.)

## Strict output sections

Return exactly these sections, in order:

- **Field selection and filters** — Bullet list (max 6 bullets); must state **`Rocket` = `Vostok`** explicitly.
...
Among space missions in the **retrieved context** where **`Rocket` is exactly `Vostok`**, what is the **distribution of mission outcomes** (`MissionStatus`), and how does that mix look in a **single Mermaid pie chart** with counts or shares clearly derived from those rows?
