...
| **Space missions MCP + RAG bot** | `SpaceMissions.McpServer` + `Chatbot` | **Eight** stdio MCP tools query **`dataset/space_missions.csv`** in-process ([Space Missions MCP guide](applications/space-missions-mcp/space-missions-mcp.md)). **`Chatbot`** builds a **RAG index** at startup, retrieves top‑K chunks per message, and hosts the MCP tool loop for Teams / playground bots; **.NET only** (no Node for this MCP). |
