...
A **.NET 8** solution ([`src/PromptEngineering.sln`](../src/PromptEngineering.sln)) for three complementary tracks:
...
All three tracks use **`PromptEngineering.LLM`** (HTTP to a DIAL- or OpenAI-compatible API).

## Shared platform capabilities

- **Per-call routing**: **`PromptEngineering.Client`** reads **`InstanceName`** and **`Temperature`** from each prompt JSON; **`Rag`** and **`Agent`** resolve **`Rag:InstanceName`** / **`Agent:InstanceName`** against the same **`Instances`** list in **`appsettings.json`** / user secrets.
...
3. [Prompt chain](applications/prompt-engineering/prompt-chain.md) · [RAG guide](applications/rag/rag.md) · for **Agent**: [README — Agent](../README.md#agent-mcp-tools), [`src/Agent/appsettings.json`](../src/Agent/appsettings.json), [agent benchmark](applications/agent/agent-weather-news.md).
